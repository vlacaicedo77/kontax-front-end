...
@Component({
  selector: 'app-cambiar-contrasena-interno',
  templateUrl: './cambiar-contrasena-interno.component.html',
  styleUrls: []
})
export class CambiarContrasenaInternoComponent implements OnInit {

  // Objeto que maneja el formulario.
  formulario: FormGroup;
  regExpPassword :RegExp = mascaras.MASK_PASSWORD_INT;
  encriptar: any;
  ...
  cambiarContrasena() {
   ...
   if(this.formulario.value.nuevo_password !== this.formulario.value.confirmar_password){
    ...
    if (!this.regExpPassword.test(this.formulario.value.nuevo_password)){
      formularioInvalido = true;
      mensaje += "<li>"+mascaras.MASK_FORMAT_MESSAGE_INT+"</li>";
    }
  ...
  if ( this.formulario.invalid || formularioInvalido) {
   ...
   return;
    ...
    Swal.fire({
      ...
      this._servicioUsuario.cambiarContraseñaUsuarioInterno(parseInt(localStorage.getItem('idUsuario')), claveEncriptada, claveAnteriorEncriptada)
        .subscribe( (resp: any) => {
          if ( resp.estado === 'OK') {
