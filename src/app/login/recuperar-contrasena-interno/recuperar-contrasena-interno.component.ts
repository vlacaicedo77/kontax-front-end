...
@Component({
  selector: 'app-recuperar-contrasena-interno',
  templateUrl: './recuperar-contrasena-interno.component.html',
  styleUrls: []
})
export class RecuperarContrasenaInternoComponent implements OnInit {
 ...
 inicializarFormulario() {
  this.formulario = new FormGroup({
    identificacion: new FormControl(null, [Validators.minLength(10), Validators.maxLength(10)]),
...
recuperarContrasena() {
 ...
 if (!this.regExpCedula.test(this.formulario.value.identificacion)){
    formularioInvalido = true;
    mensaje += "<li>La Cédula no tiene el formado correcto</li>";
  }
...
if ( this.formulario.invalid || formularioInvalido) {
 ...
 return;
  ...
  Swal.fire({
    ...
    this._servicioUsuario.recuperarContraseñaUsuarioInterno(this.formulario.value.identificacion,this.formulario.value.email)
      .subscribe((resp: any) => {
        if ( resp.estado === 'OK') {
