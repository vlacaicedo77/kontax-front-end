...
@Component({
  selector: 'app-asignar-arete-bovino-operador',
  templateUrl: './asignar-arete-bovino-operador.component.html'
})
export class AsignarAreteBovinoOperadorComponent implements OnInit {
  ...
  buscarSitiosProductor() {
    ...
    if (this.formularioBusqueda.value.inputIdentificacion.trim() == this.usuarioServicio.usuarioExterno.numeroIdentificacion) {
      Swal.fire({
        title: '¡Advertencia!',
        text: 'La autoasignación no está permitida.',
        icon: 'warning',
        confirmButtonText: 'Ok',
        timer: 5000,
        showCancelButton: false
  ...
  buscarAnimales(mensaje: boolean = false, inicioDefault: boolean = false) {
    ...
    if (this.formularioBusqueda.value.inputSitio) {
      ...
      parametros.idUsuarioActual = this.sitioSeleccionado.idUsuariosExternos;//this.usuarioServicio.usuarioExterno.idUsuario;
  ...
  buscarAretesOficiales(mensaje: boolean = false, limite: boolean = false) {
    ...
    const idUsuarioActual = (parseInt(localStorage.getItem('idUsuario') || '0', 10) || 0);
  ...
  // Método auxiliar para restablecer fecha (opcional, para reutilizar código)
  ...
  actualizarAnimal(id: number) {
    ...
    // Otras Vacunas
    if (this.formulario.value.inputIdVacuna != '-1') {
      animal.idVacunasOficiales = this.formulario.value.inputIdVacuna;
      animal.idUsuarioProductor = this.animalSeleccionado.idUsuarioActual;
      animal.idSitio = this.animalSeleccionado.idSitioActual;
      animal.fechaVacunacion = this.formulario.value.inputFechaVacunacion;
    }
  ...
  exportarListadoAretesPDF() {
    ...
    this.reportesAretesService.exportAsPdfFileAretesListado(pdfData,true);
    Swal.close();
    Swal.fire({
