...
@Component({
  selector: 'app-registro-ferias-comercializacion',
  templateUrl: './registro-ferias-comercializacion.component.html',
  styleUrls: ['./registro-ferias-comercializacion.css']
})
export class RegistroFeriasComercializacionComponent implements OnInit {
  ...
  registrarFeriaComercializacion() {
  ...
  Swal.fire({
    title: 'Está seguro de registrar la feria de comercialización?',
    ...
    if (result.value) {
      ...
      this._areaService.registrarFeriaComercializacion(area)
      .subscribe( (resp: any) => {
        if ( resp.estado === 'OK') {
          Swal.fire('Éxito', 'La Feria de Comercialización fue registrada correctamente', 'success');
  ...
  // Método que obtiene el listado de ferias de comercialización del sistema GUIA
  buscarFeriasComercializacion(numeroIdentificacion: string) {
    this.feriasVisible = false;
    if ( numeroIdentificacion === null) {
      return;
    }
    this._sitioService.consultarFeriasComercializacionGUIAPorNumeroIdentificacionUsuario(numeroIdentificacion)
  .subscribe( (resp: any) => {
    if ( resp.estado === 'OK')  {
      if(resp.resultado.length > 0) {
        ...
        Swal.fire('Éxito', 'El productor no tiene ferias de comercialización activas en el sistema GUIA', 'success');
      }
    }
    else {
      Swal.fire('Error', resp.mensaje , 'error');
