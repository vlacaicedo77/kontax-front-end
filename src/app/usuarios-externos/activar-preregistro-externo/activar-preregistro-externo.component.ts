...
@Component({
  selector: 'app-activar-preregistro-externo',
  templateUrl: './activar-preregistro-externo.component.html',
  styleUrls: ['./activar-preregistro-externo.component.css']
})
export class ActivarPreregistroExternoComponent implements OnInit {
  ...
  inicializarFormulario() {
    this.formulario = new FormGroup({
      ...
      //inputRazonSocial: new FormControl(null),
    ...
    buscarUsuario()
  {
    ...
    this._servicioUsuario.consultarUsuarioExtFiltros(null, null, null, identificacionUsuario, 1, null)
    .subscribe( (resp: any) => {
      if (resp.estado === 'OK') {
 ...
 /* validarRuc(ruc: string){
    ...
    this.formulario.controls.inputRazonSocial.setValue(null);
    Swal.fire({
      title: 'Espere...',
    ...
    this.servicioDinardap.obtenerUbicacionesSri(ruc).subscribe(
      ( respuesta: UbicacionSri[]) => {
        ...
        respuesta.forEach( (item: UbicacionSri) => {
          this.formulario.controls.inputRazonSocial.setValue(item.razonSocial);
        });
      }
    );
  }*/
  ...
  activarUsuario()
  {
    ...
    Swal.fire({
      title: '¡Atención!',//'Contraseña Temporal: '+this.contrasenaTemporal,
      //text: 'Yo, '+this.nombresUsuarioInterno+' con C.C./C.I.: '+ this.idUsuarioInterno+', relizaré la activación de la cuenta '+this.identificacion+' a petición del titular o representante legal, quien se encuentra presente. Asimismo cuento con documentos de soporte si se llegase a requirir pruebas de descargo. En caso de mal uso de esta herramienta, la Agencia podrá actuar en derecho y tomar acciones de acuerdo a la ley.',//"Esta contraseña se enviará al correo electrónico registrado. Si no se encuentra en bandeja de entrada, favor indicar al usuario que debe revisar su bandeja de correo no deseado o SPAM",
      text: 'Yo, '+this.nombresUsuarioInterno+' con C.C./C.I.: '+ this.idUsuarioInterno+', relizaré la activación de la cuenta '+this.identificacion+' a petición del titular o representante legal, quien se encuentra presente.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'He leído y estoy de acuerdo',//'Sí, continuar',
      ...
      if (result.value) {
        ...
        Swal.fire({
          title: 'Espere...',
          text: 'Registrando transacción en la base de datos... Notificando al solicitante ['+this.formulario.value.inputEmail.toLowerCase()+'] con copia al funcionario modificador ['+this.emailUsuarioInterno+']...',//'Activando cuenta de usuario '+this.identificacion+'...',
          confirmButtonText: '',
          allowOutsideClick: false,
          onBeforeOpen: () => {
              Swal.showLoading();
      ...
      usuario.nombres = this.formulario.value.inputNombres;
      usuario.apellidos = this.formulario.value.inputNombres;
      ...
      this._servicioUsuario.activarUsuarioExterno(usuario)
        .subscribe((resp: any) => {
          if (resp.estado === 'OK') {
            Swal.fire('Éxito', 'La cuenta de usuario '+this.identificacion+' se ha activado correctamente. La contraseña temporal ha sido notificada al correo ['+this.formulario.value.inputEmail+']', 'success');
  ...
  /*cambiarValidacionesIdentificacion(tipoIdentificacion :string)
  {
    ...
    {
      this.formulario.controls.inputRazonSocial.setValidators([Validators.pattern(mascaras.MASK_ALFANUMERICO)]);
      this.formulario.controls.inputRazonSocial.updateValueAndValidity();
