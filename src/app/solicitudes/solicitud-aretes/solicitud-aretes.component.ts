...
@Component({
  selector: 'app-solicitud-aretes',
  templateUrl: './solicitud-aretes.component.html',
  styleUrls: ['./solicitud-aretes.component.css']
})
export class SolicitudAretesComponent implements OnInit {
  ...
  inicializarFormulario() {
    ...
    this.formulario = new FormGroup({
      ...
      inputTipoArete: new FormControl('1', [Validators.required]),
  ...
  buscarSolicitudes() {
    ...
    parametros.idTiposSolicitantes = 1;
    ...
    this.solicitudAretesService.consultarEstadisticasValidaciones(idUsuario)
      .subscribe((resp: any) => {
        if (resp.estado === 'OK') {
          ...
          this.validacionAretesCatastro = this.validacionAretes.disponibles;
          //this.formularioVisible = true;
        }
        else {
          //this.formularioVisible = false;
  ...
  registrarSolicitudAretes() {
    ...
    if (this.formulario.value.inputCantidad > this.validacionAretesCatastro) {
      formularioInvalido = true;
      mensaje += "<li>El número máximo de aretes que puede solicitar es [" + this.validacionAretesCatastro + "]</li>";
    }
    ...
    solicitud.idTiposSolicitantes = 1; // productor (ganadero)
  ...
  obtenerTiposAretes() {
    ...
    this.mostrarCargando('Cargando datos...');
    this.listaTiposAretes = [];
    this.tiposAretesService.obtenerTiposAretes({ estado: 1 })
      .subscribe((tiposAretes: TipoArete[]) => {
        // Filtrar solo id_tipos_aretes = 1
        this.listaTiposAretes = tiposAretes.filter(tipo => tipo.id_tipos_aretes === 1);

        // Igualar this.nombreTipoArete con la clave nombre del resultado
        if (this.listaTiposAretes.length > 0) {
          this.nombreTipoArete = this.listaTiposAretes[0].nombre;
        }

        Swal.close();
      }, (error) => {
        Swal.fire('Error', 'No se pudo obtener los tipos de aretes. Intente nuevamente más tarde: ' + error, 'error');
      });
  }
