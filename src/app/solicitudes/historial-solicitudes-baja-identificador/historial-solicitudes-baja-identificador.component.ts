...
@Component({
  selector: 'app-historial-solicitudes-baja-identificador',
  templateUrl: './historial-solicitudes-baja-identificador.component.html',
  styleUrls: []
})
export class HistorialSolicitudesBajaIdentificadorComponent implements OnInit {
  ...
  .subscribe( (resp: any) => {
   ...
   buscarHistoricoSolicitud(solicitud){
    this.solicitudSeleccionada = solicitud;
    this._solicitudService.consultarHistoricoSolicitud(this.solicitudSeleccionada['id_solicitudes_baja_identificador'])
  .subscribe( (resp: any) => {
    if ( resp.estado === 'OK')  
    {
      //console.log (resp);
      this.detalleVisible = true;
      this.historicoSolicitud = resp.resultado.historico;
      Swal.fire('Éxito', 'Se ha realizado la búsqueda exitosamente', 'success');
    }
    else {
