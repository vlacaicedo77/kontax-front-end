...
@Component({
  selector: 'app-historial-solicitudes-reareteo',
  templateUrl: './historial-solicitudes-reareteo.component.html',
  styleUrls: []
})
export class HistorialSolicitudesReareteoComponent implements OnInit {
  ...
  .subscribe( (resp: any) => {
   ...
   buscarDetalleSolicitud(solicitud){
    this.solicitudSeleccionada = solicitud;
    this._solicitudService.consultarDetalleSolicitud(this.solicitudSeleccionada['id_solicitudes_reareteo'])
  .subscribe( (resp: any) => {
    if ( resp.estado === 'OK')  
    {
      //console.log (resp);
      ...
      this.bovino = resp.resultado.bovino;
      Swal.fire('Éxito', 'Se ha realizado la búsqueda exitosamente', 'success');
    }
    else {
